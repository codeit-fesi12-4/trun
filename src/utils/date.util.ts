...
// Date 객체를 "yyyy-MM-dd hh:mm a" 형식으로 변환 (DatePicker용, AM/PM 표시)
export const formatDatePicker = (dateValue: Date | undefined): string => {
  if (!dateValue) return "";
  return format(dateValue, "yyyy-MM-dd hh:mm a", { locale: ko });
};

// Date 객체를 "yyyy-MM-dd" 형식으로 변환 (필터용, API 파라미터용)
export const formatDateWithDash = (date: Date | undefined): string | undefined => {
  if (!date) return undefined;
  return format(date, "yyyy-MM-dd");
};

// ISO 8601 날짜 문자열을 "M월 d일" 형식으로 변환
